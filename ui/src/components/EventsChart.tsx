...
export function EventsChart({ events, selectedMinute, onMinuteClick }: EventsChartProps) {
  const [barHovered, setBarHovered] = useState(false);

  const chartData = useMemo(() => {
    // Group events by minute
    const minuteGroups = new Map<string, { valid: number; failed: number; timestamp: number }>();

    events.forEach(event => {
      if (!event.collector_tstamp) return;

      const timestamp = new Date(event.collector_tstamp).getTime();
      const minuteKey = new Date(Math.floor(timestamp / 60000) * 60000).toISOString();
      ...
      if (!minuteGroups.has(minuteKey)) {
        minuteGroups.set(minuteKey, { valid: 0, failed: 0, timestamp: Math.floor(timestamp / 60000) * 60000 });
      }
        ...
        >
          <defs>
            <pattern id="failedPattern" patternUnits="userSpaceOnUse" width="6" height="6">
              <rect width="6" height="6" fill="var(--dark-orange)"/>
              <line x1="0" y1="0" x2="6" y2="6" stroke="var(--very-dark-orange)" strokeWidth="0.5" opacity="0.6"/>
              <line x1="0" y1="6" x2="6" y2="0" stroke="var(--very-dark-orange)" strokeWidth="0.5" opacity="0.6"/>
            </pattern>
          </defs>
            <XAxis
              dataKey="minute"
