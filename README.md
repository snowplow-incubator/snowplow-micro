...
The distroless image is tagged as `snowplow/snowplow-micro:1.3.4-distroless` on Docker Hub.
...
```bash
java -jar snowplow-micro-1.3.4.jar --collector-config example/micro.conf --iglu example/iglu.json
```
...
```bash
# Unix
java -cp snowplow-micro-1.3.4.jar:example com.snowplowanalytics.snowplow.micro.Main --collector-config example/micro.conf --iglu example/iglu.json
# Windows
java -cp snowplow-micro-1.3.4.jar;example com.snowplowanalytics.snowplow.micro.Main --collector-config example/micro.conf --iglu example/iglu.json
```
