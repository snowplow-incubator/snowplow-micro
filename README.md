...
```
$ docker run --mount type=bind,source=$(pwd)/example,destination=/config -p 9090:9090 snowplow-docker-registry.bintray.io/snowplow/micro:latest --collector-config /config/micro.conf --iglu /config/iglu.json
```
