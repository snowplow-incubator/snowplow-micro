...
```bash
docker run -p 9090:9090 snowplow/snowplow-micro:1.3.5
```
...
The distroless image is tagged as `snowplow/snowplow-micro:1.3.5-distroless` on Docker Hub.
...
```bash
java -jar snowplow-micro-1.3.5.jar --collector-config example/micro.conf --iglu example/iglu.json
```
...
```bash
# Unix
java -cp snowplow-micro-1.3.5.jar:example com.snowplowanalytics.snowplow.micro.Main --collector-config example/micro.conf --iglu example/iglu.json
# Windows
java -cp snowplow-micro-1.3.5.jar;example com.snowplowanalytics.snowplow.micro.Main --collector-config example/micro.conf --iglu example/iglu.json
```
